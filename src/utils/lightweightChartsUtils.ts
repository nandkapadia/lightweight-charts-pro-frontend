...
export function asLineStyle(value: number | string): any {
  ...
  // If value is a string, parse it to integer (base 10)
...
// ============================================================================
// Comprehensive Options Converter
// ============================================================================
...
export function createSeriesOptions(
  ...
  // Create new object to hold converted options
  const converted: Record<string, any> = {};

  // Iterate through all properties in input options
  for (const [key, value] of Object.entries(options)) {
    // Convert special properties using type-safe converters
    switch (key) {
      // Line width properties
      case "lineWidth":
      case "priceLineWidth":
      case "baseLineWidth":
        // Convert to type-safe line width
      ...
      case "priceLineStyle":
      case "baseLineStyle":
        // Convert to type-safe line style (handles string/number)
        converted[key] = asLineStyle(value);
        break;

      // Price line source property
      case "priceLineSource":
        // Convert to type-safe price line source
...
export function safeSeriesOptions<T>(options: T): T {
  // Pass through with type assertion for TypeScript compatibility
  return options as T;
}
