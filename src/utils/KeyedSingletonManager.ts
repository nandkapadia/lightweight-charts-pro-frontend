...
// ============================================================================
// Keyed Singleton Manager Base Class
// ============================================================================
 ...
 * ```
 */
export abstract class KeyedSingletonManager<
  _T extends Destroyable,
> implements Destroyable {
  ...
  protected static getOrCreateInstance<T>(
    ...
    // Check if an instance exists for this key
    if (!instanceMap.has(key)) {
      // No instance exists - create one using the factory function
      // The factory is responsible for calling the constructor with appropriate args
      instanceMap.set(key, factory());
    }

    // Return the instance (guaranteed to exist now)
  ...
  protected static destroyInstanceByKey(className: string, key: string): void {
    // Get the instance map for this class
    const instanceMap = KeyedSingletonManager.instanceMaps.get(className);
    ...
    if (instanceMap && instanceMap.has(key)) {
      ...
      // Call destroy() if the instance has this method
      // This check is for safety, though all instances should have destroy()
    ...
    // If instance doesn't exist, this is a no-op (safe to call)
  }
  ...
  protected static hasInstanceWithKey(className: string, key: string): boolean {
    ...
    // Return true if map exists and has the key, false otherwise
  ...
  protected static getInstanceKeys(className: string): string[] {
    ...
    // Return array of keys if map exists, empty array otherwise
  ...
  protected static clearAllInstances(className: string): void {
    ...
    if (instanceMap) {
      // Destroy each instance before clearing the map
      ...
      // Clear the map (removes all entries)
      instanceMap.clear();
    }
  }
...
export function createInstanceKey(
  ...
  // Map each part to a string:
  // - undefined → 'default'
  // - numbers → string representation
  // - strings → as-is
  // Then join all parts with hyphens
  return parts
