...
export class EventEmitter {
  ...
  on(event: string, listener: EventListener): this {
    // Check if this event has any listeners yet
    if (!this.events.has(event)) {
      // First listener for this event - create a new array
      this.events.set(event, []);
    }

    // Add the listener to the array for this event
    // We know this exists because we just created it if needed
    const listeners = this.events.get(event);
    if (listeners) {
      listeners.push(listener);
    }

    // Return 'this' to allow method chaining (e.g., emitter.on(...).on(...))
    return this;
  }
  ...
  once(event: string, listener: EventListener): this {
    // Create a wrapper function that will remove itself after execution
    const onceWrapper = (...args: any[]) => {
      // First, execute the original listener with all the arguments
      listener(...args);

      // Then remove this wrapper from the event listeners
      // We remove the wrapper, not the original listener
      this.off(event, onceWrapper);
    };

    // Register the wrapper function (not the original listener)
  ...
  off(event: string, listener: EventListener): this {
    // Get the array of listeners for this event (may be undefined if event doesn't exist)
    const listeners = this.events.get(event);

    if (listeners) {
      // Find the index of this listener in the array
      const index = listeners.indexOf(listener);

      if (index !== -1) {
        // Listener found - remove it from the array
        // splice() modifies the array in place
        listeners.splice(index, 1);
      }

      // If this was the last listener, clean up the empty array to save memory
  ...
  emit(event: string, ...args: any[]): boolean {
    ...
    if (listeners && listeners.length > 0) {
      // Execute each listener in order
      listeners.forEach((listener) => {
        try {
          // Call the listener with all the provided arguments
          listener(...args);
        } catch (error) {
          // If a listener throws an error, log it but continue with other listeners
          // This prevents one broken listener from breaking the entire event system
          console.error(`Error in event listener for '${event}':`, error);
        }
      });

      // Return true to indicate the event had listeners
      return true;
    }

    // Return false to indicate no listeners were called
    return false;
  }
  ...
  listenerCount(event: string): number {
    // Get the listeners array and return its length
  ...
  listeners(event: string): EventListener[] {
    // Get the listeners array and make a shallow copy with .slice()
    // If no listeners exist, return an empty array
  ...
  setMaxListeners(n: number): this {
    // Store the value for API compatibility
    this.maxListeners = n;
