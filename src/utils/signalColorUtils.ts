...
export class SignalColorCalculator {
  ...
  static checkForNonBooleanValues(values: number[]): boolean {
    ...
    for (const value of values) {
      // Convert to number to handle both boolean and numeric types
      // Number(false) = 0, Number(true) = 1, Number(0) = 0, etc.
      const numValue = Number(value);

      // Check if value is anything other than 0 or 1
      if (numValue !== 0 && numValue !== 1) {
        // Found a non-boolean value (e.g., -1, 2, 0.5, etc.)
        // Return true immediately without checking remaining values
        return true;
      }
    }

    // All values are 0 or 1 - this is boolean-only data
    return false;
  }
  ...
  static getColorForValue(
    ...
    // Convert value to number to handle both boolean and numeric types
    // This ensures true → 1, false → 0, and numbers stay as numbers
    const numValue = Number(value);

    // Check if value is zero (neutral state)
    if (numValue === 0) {
      // Return neutralColor, or 'transparent' if not provided
      return options.neutralColor || "transparent";
    } else if (numValue > 0) {
      // Positive value (bullish signal)
      // Return signalColor, or 'transparent' if not provided
      return options.signalColor || "transparent";
    } else {
      // Negative value (bearish signal or data error)

      // Only use alertColor if dataset contains non-boolean values
      ...
      if (hasNonBooleanValues) {
        // Dataset has negative values - use alertColor
        // Fallback chain: alertColor → signalColor → 'transparent'
        return options.alertColor || options.signalColor || "transparent";
      } else {
        // Dataset is boolean-only - negative values are unexpected
        // Use signalColor as failsafe (or 'transparent' if not provided)
        return options.signalColor || "transparent";
      }
    }
  }
