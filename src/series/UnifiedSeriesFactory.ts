...
export function createSeries(
  ...
  try {
    ...
    const { displayName, ...apiOptions } = flattenedUserOptions as Record<
      string,
      unknown
    > & { displayName?: string };
...
export function createSeriesWithConfig(
  ...
  try {
    ...
    const mergedOptions = title !== undefined ? { ...options, title } : options;
      ...
      priceLines.forEach((priceLine: Record<string, unknown>) => {
        try {
          ...
          series.createPriceLine(
            priceLine as Parameters<typeof series.createPriceLine>[0],
          );
        } catch (error) {
          logger.warn(
