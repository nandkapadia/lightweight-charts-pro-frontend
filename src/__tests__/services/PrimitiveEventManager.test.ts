...
describe("PrimitiveEventManager", () => {
  ...
  describe("Chart Event Integration", () => {
    it("should emit crosshairMove when chart crosshair moves", () => {
      ...
      const crosshairHandler = (mockChart.subscribeCrosshairMove as any).mock
        .calls[0][0];
      crosshairHandler({
        time: 100,
        point: { x: 10, y: 20 },
    ...
    it("should emit timeScaleChange when time scale changes", () => {
      ...
      const timeScaleHandler =
        mockTimeScale.subscribeVisibleTimeRangeChange.mock.calls[0][0];
      timeScaleHandler();
  ...
  describe("Destruction and Cleanup", () => {
    ...
    it("should unsubscribe from chart events on destroy", () => {
      ...
      expect(
        mockTimeScale.unsubscribeVisibleTimeRangeChange,
      ).toHaveBeenCalled();
    });
