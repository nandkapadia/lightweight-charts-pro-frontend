...
const defaultBandOptions = {
  ...
  upperLineWidth: 2,
  upperLineStyle: signalColorUtils.LineStyle.Solid,
  upperLineVisible: true,
  middleLineColor: "#2196F3",
  middleLineWidth: 2,
  middleLineStyle: signalColorUtils.LineStyle.Solid,
  middleLineVisible: true,
  lowerLineColor: "#F44336",
  lowerLineWidth: 2,
  lowerLineStyle: signalColorUtils.LineStyle.Solid,
...
class BandSeries {
  ...
  isWhitespace(data) {
    return TrendFillPrimitive.isWhitespaceDataMultiField(data, ["upper", "middle", "lower"]);
  }
  renderer() {
...
class BandSeriesRenderer {
  ...
  _drawImpl(renderingScope, priceToCoordinate) {
    ...
    if (options.upperFill) {
      TrendFillPrimitive.drawFillArea(
        ctx,
        bars,
        "upperY",
        "middleY",
...
function createBandSeries(chart, options = {}) {
  ...
  const series = chart.addCustomSeries(new BandSeries(), {
    ...
    upperLineStyle: options.upperLineStyle ?? signalColorUtils.LineStyle.Solid,
    ...
    middleLineStyle: options.middleLineStyle ?? signalColorUtils.LineStyle.Solid,
    ...
    lowerLineStyle: options.lowerLineStyle ?? signalColorUtils.LineStyle.Solid,
  ...
  if (options.usePrimitive) {
    void Promise.resolve().then(() => require("./BandPrimitive-B-FZ-yCS.cjs")).then(({ BandPrimitive }) => {
      const primitive = new BandPrimitive(chart, {
        upperLineColor: options.upperLineColor ?? "#4CAF50",
        upperLineWidth: options.upperLineWidth ?? 2,
        upperLineStyle: Math.min(options.upperLineStyle ?? signalColorUtils.LineStyle.Solid, 2),
        upperLineVisible: options.upperLineVisible !== false,
        middleLineColor: options.middleLineColor ?? "#2196F3",
        middleLineWidth: options.middleLineWidth ?? 2,
        middleLineStyle: Math.min(options.middleLineStyle ?? signalColorUtils.LineStyle.Solid, 2),
        middleLineVisible: options.middleLineVisible !== false,
        lowerLineColor: options.lowerLineColor ?? "#F44336",
        lowerLineWidth: options.lowerLineWidth ?? 2,
        lowerLineStyle: Math.min(options.lowerLineStyle ?? signalColorUtils.LineStyle.Solid, 2),
        lowerLineVisible: options.lowerLineVisible !== false,
        upperFillColor: options.upperFillColor ?? "rgba(76, 175, 80, 0.1)",
        upperFill: options.upperFill !== false,
        // Changed from upperFillVisible
      ...
      SingletonBase.logger.error("Failed to load BandPrimitive", "BandSeries", error);
    });
  }
  return series;
}
...
class RibbonSeries {
  ...
  isWhitespace(data) {
    return TrendFillPrimitive.isWhitespaceDataMultiField(data, ["upper", "lower"]);
  }
  renderer() {
    return this._renderer;
  }
...
function createRibbonSeries(chart, options = {}) {
  ...
  if (options.usePrimitive) {
    void Promise.resolve().then(() => require("./RibbonPrimitive-CTwl-n6e.cjs")).then(({ RibbonPrimitive }) => {
      ...
      SingletonBase.logger.error("Failed to load RibbonPrimitive", "RibbonSeries", error);
    });
  }
  return series;
}
...
class GradientRibbonSeriesRenderer {
  ...
  _convertToScreenCoordinates(scope, priceConverter) {
    ...
    const baseCoordinates = coordinateService.convertSeriesDataToScreenCoordinates(
      ...
      scope,
      priceConverter,
      SingletonBase.ChartCoordinateService.SeriesDataConfigs.gradientRibbon
    );
    let maxSpread = 0;
    let minGradient = 0;
    let maxGradient = 1;
...
function createGradientRibbonSeries(chart, options = {}) {
  ...
  if (options.usePrimitive) {
    void Promise.resolve().then(() => require("./GradientRibbonPrimitive-CS1Z2c97.cjs")).then(({ GradientRibbonPrimitive }) => {
      ...
      SingletonBase.logger.error("Failed to load GradientRibbonPrimitive", "GradientRibbonSeries", error);
    });
  }
  return series;
}
...
class SignalSeriesRenderer {
  ...
  _checkForNonBooleanValues(data) {
    ...
    return signalColorUtils.SignalColorCalculator.checkForNonBooleanValues(values);
  }
  draw(target) {
    target.useBitmapCoordinateSpace((scope) => {
  ...
  _drawImpl(renderingScope) {
    ...
    for (const bar of this._data.bars) {
      ...
      if (signalColorUtils.isTransparent(color)) {
        continue;
      }
  ...
  getColorForValue(value, options) {
    return signalColorUtils.SignalColorCalculator.getColorForValue(value, options, this._hasNonBooleanValues);
  }
}
class SignalSeries {
  constructor() {
...
function createSignalSeries(chart, options = {}) {
  ...
  if (usePrimitive) {
    void Promise.resolve().then(() => require("./SignalPrimitive-DKmHoKb-.cjs")).then(({ SignalPrimitive }) => {
      ...
      SingletonBase.logger.error("Failed to load SignalPrimitive", "SignalSeries", error);
    });
  }
  return series;
}
...
class TrendFillSeriesRenderer {
  ...
  _applyLineStyle(ctx, style) {
    switch (style) {
      case signalColorUtils.LineStyle.Solid:
        ctx.setLineDash([]);
        break;
      case signalColorUtils.LineStyle.Dotted:
        ctx.setLineDash([1, 1]);
        break;
      case signalColorUtils.LineStyle.Dashed:
        ctx.setLineDash([4, 2]);
        break;
      case signalColorUtils.LineStyle.LargeDashed:
        ctx.setLineDash([8, 4]);
        break;
      case signalColorUtils.LineStyle.SparseDotted:
        ctx.setLineDash([1, 4]);
        break;
      default:
        ctx.setLineDash([]);
...
class TrendFillSeries {
   ...
   */
  isWhitespace(data) {
    return TrendFillPrimitive.isWhitespaceDataMultiField(data, ["baseLine", "trendLine"]);
  }
  renderer() {
    return this._renderer;
  }
  ...
  defaultOptions() {
    return {
      ...
      // Green for uptrend
      uptrendLineWidth: 2,
      uptrendLineStyle: signalColorUtils.LineStyle.Solid,
      uptrendLineVisible: true,
      downtrendLineColor: "#F44336",
      // Red for downtrend
      downtrendLineWidth: 2,
      downtrendLineStyle: signalColorUtils.LineStyle.Solid,
      downtrendLineVisible: true,
      baseLineColor: "#666666",
      baseLineWidth: 1,
      baseLineStyle: signalColorUtils.LineStyle.Dotted,
...
function createTrendFillSeries(chart, options = {}) {
  const series = chart.addCustomSeries(new TrendFillSeries(), {
    ...
    uptrendLineStyle: options.uptrendLineStyle ?? signalColorUtils.LineStyle.Solid,
    ...
    downtrendLineStyle: options.downtrendLineStyle ?? signalColorUtils.LineStyle.Solid,
    ...
    baseLineStyle: options.baseLineStyle ?? signalColorUtils.LineStyle.Dotted,
  ...
  if (options.usePrimitive) {
    const primitiveOptions = {
      ...
      uptrendLineColor: options.uptrendLineColor ?? "#4CAF50",
      uptrendLineWidth: options.uptrendLineWidth ?? 2,
      uptrendLineStyle: Math.min(options.uptrendLineStyle ?? signalColorUtils.LineStyle.Solid, 2),
      uptrendLineVisible: options.uptrendLineVisible !== false,
      // Downtrend line options (flat)
      downtrendLineColor: options.downtrendLineColor ?? "#F44336",
      downtrendLineWidth: options.downtrendLineWidth ?? 2,
      downtrendLineStyle: Math.min(options.downtrendLineStyle ?? signalColorUtils.LineStyle.Solid, 2),
      downtrendLineVisible: options.downtrendLineVisible !== false,
      // Base line options (flat)
      baseLineColor: options.baseLineColor ?? "#666666",
      baseLineWidth: options.baseLineWidth ?? 1,
      baseLineStyle: Math.min(options.baseLineStyle ?? signalColorUtils.LineStyle.Dotted, 2),
      baseLineVisible: options.baseLineVisible === true,
      visible: true,
      priceScaleId: options.priceScaleId ?? "right",
    ...
    const primitive = new TrendFillPrimitive.TrendFillPrimitive(chart, primitiveOptions);
    series.attachPrimitive(primitive);
    if (options.data && options.data.length > 0) {
...
exports.SignalSeries = SignalSeries;
exports.SignalSeriesPlugin = SignalSeriesPlugin;
exports.createBandSeries = createBandSeries;
exports.createGradientRibbonSeries = createGradientRibbonSeries;
exports.createRibbonSeries = createRibbonSeries;
exports.createSignalSeries = createSignalSeries;
exports.createSignalSeriesPlugin = createSignalSeriesPlugin;
exports.createTrendFillSeries = createTrendFillSeries;
exports.defaultSignalOptions = defaultSignalOptions;
//# sourceMappingURL=trendFillSeriesPlugin-DogJ13XL.cjs.map
