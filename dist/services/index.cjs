...
class PaneCollapseManager extends Disposable.KeyedSingletonManager {
  constructor(chartApi, config = {}) {
    ...
    this.config = {
      collapsedHeight: SingletonBase.DIMENSIONS.pane.collapsedHeight,
  ...
  static getInstance(chartApi, chartId, config = {}) {
    ...
    return Disposable.KeyedSingletonManager.getOrCreateInstance(
      "PaneCollapseManager",
      key,
  ...
  static destroyInstance(chartId) {
    const key = chartId || "default";
    Disposable.KeyedSingletonManager.destroyInstanceByKey("PaneCollapseManager", key);
  }
  /**
  ...
  initializePane(paneId, callbacks) {
    if (!this.states.has(paneId)) {
      this.states.set(paneId, {
        isCollapsed: false,
        originalHeight: 0,
        collapsedHeight: this.config?.collapsedHeight || SingletonBase.DIMENSIONS.pane.collapsedHeight,
  ...
  toggle(paneId) {
    ...
    try {
      ...
      errorHandler.handleError(error, "PaneCollapseManager.toggle", errorHandler.ErrorSeverity.WARNING);
    }
  }
  /**
  ...
  collapse(paneId) {
    ...
    try {
      ...
      errorHandler.handleError(error, "PaneCollapseManager.collapse", errorHandler.ErrorSeverity.ERROR);
    }
  }
  /**
  ...
  expand(paneId) {
    ...
    try {
      ...
      errorHandler.handleError(error, "PaneCollapseManager.expand", errorHandler.ErrorSeverity.ERROR);
    }
  }
  /**
   * Cleanup resources
  ...
  destroy() {
    this.states.clear();
    Disposable.cleanupInstance(this, ["chartApi", "config"]);
  }
}
...
exports.convertTradeRectanglesToPluginFormat = tradeVisualization.convertTradeRectanglesToPluginFormat;
exports.convertTradeRectanglesToPluginFormatWhenReady = tradeVisualization.convertTradeRectanglesToPluginFormatWhenReady;
exports.createTradeVisualElements = tradeVisualization.createTradeVisualElements;
exports.PaneCollapseManager = PaneCollapseManager;
exports.createAnnotationLayer = createAnnotationLayer;
exports.createAnnotationVisualElements = createAnnotationVisualElements;
exports.filterAnnotationsByPriceRange = filterAnnotationsByPriceRange;
exports.filterAnnotationsByTimeRange = filterAnnotationsByTimeRange;
