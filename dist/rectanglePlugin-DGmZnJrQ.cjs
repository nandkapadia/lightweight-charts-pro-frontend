...
class TooltipPlugin {
  ...
  constructor(container, _chartId) {
    ...
    TooltipManager.TooltipManager.getInstance().registerRenderer(this);
  }
  /**
  ...
  show(content, style, position, cssClasses) {
    try {
      ...
      if (!this.tooltipElement) {
        SingletonBase.logger.warn("Failed to create tooltip element", "TooltipPlugin");
        return;
      }
      this.tooltipElement.innerHTML = sanitization.sanitizeHtml(content);
      ...
      SingletonBase.logger.error("Failed to show tooltip", "TooltipPlugin", error);
    }
  }
  /**
  ...
  applyStyle(element, style) {
    try {
      ...
      SingletonBase.logger.error("Failed to apply tooltip style", "TooltipPlugin", error);
    }
  }
  /**
  ...
  position(point) {
    ...
    try {
      ...
      SingletonBase.logger.error("Failed to position tooltip", "TooltipPlugin", error);
    }
  }
  /**
  ...
  destroy() {
    try {
      ...
      TooltipManager.TooltipManager.getInstance().unregisterRenderer();
      SingletonBase.logger.info("TooltipPlugin destroyed", "TooltipPlugin");
    } catch (error) {
      SingletonBase.logger.error("Failed to destroy TooltipPlugin", "TooltipPlugin", error);
    }
  }
  /**
...
class RectangleOverlayPlugin {
  constructor() {
    ...
    this.resizeObserverManager = new resizeObserverManager.ResizeObserverManager();
  }
  ...
  async init() {
    ...
    try {
      ...
      const isReady = await chartReadyDetection.ChartReadyDetector.waitForChartReady(this.chart, container, {
        minWidth: 200,
        minHeight: 200
      });
      if (!isReady) {
      ...
      SingletonBase.logger.error("Rectangle overlay operation failed", "RectangleOverlayPlugin", error);
    }
  }
  createCanvas() {
  ...
  calculateActualCoordinates(x1, y1, x2, y2) {
    ...
    try {
      try {
        ...
        SingletonBase.logger.error("Rectangle operation failed", "RectangleOverlayPlugin", error);
      }
      return {
        ax1: x1,
        ay1: y1,
  ...
  drawLabel(rect, x1, y1, x2, y2) {
    ...
    try {
      ...
      if (rect.labelBackground) {
        ...
        const padding = rect.labelPadding || SingletonBase.UniversalSpacing.DEFAULT_PADDING;
...
exports.RectangleOverlayPlugin = RectangleOverlayPlugin;
exports.TooltipPlugin = TooltipPlugin;
//# sourceMappingURL=rectanglePlugin-DGmZnJrQ.cjs.map
