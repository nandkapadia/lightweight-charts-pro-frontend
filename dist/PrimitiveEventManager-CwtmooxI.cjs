...
class CornerLayoutManager extends Disposable.KeyedSingletonManager {
  constructor(chartId, paneId) {
    super();
    this.config = {
      edgePadding: SingletonBase.LayoutSpacing.EDGE_PADDING,
      widgetGap: SingletonBase.LayoutSpacing.WIDGET_GAP,
      baseZIndex: SingletonBase.LayoutSpacing.BASE_Z_INDEX
    };
    this.cornerStates = {
    ...
    this.coordinateService = SingletonBase.ChartCoordinateService.getInstance();
    this.paneId = paneId;
  }
  static getInstance(chartId, paneId) {
    const key = Disposable.createInstanceKey(chartId, "pane", paneId);
  ...
  static cleanup(chartId, paneId) {
    if (paneId !== void 0) {
      const key = Disposable.createInstanceKey(chartId, "pane", paneId);
      Disposable.KeyedSingletonManager.destroyInstanceByKey("CornerLayoutManager", key);
    } else {
      const allKeys = Disposable.KeyedSingletonManager.getInstanceKeys("CornerLayoutManager");
  ...
  destroy() {
    ...
    this.events = {};
    Disposable.cleanupInstance(this, ["chartApi", "coordinateService", "config"]);
  }
}
...
exports.CornerLayoutManager = CornerLayoutManager;
exports.PrimitiveEventManager = PrimitiveEventManager;
//# sourceMappingURL=PrimitiveEventManager-CwtmooxI.cjs.map
